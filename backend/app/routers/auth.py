...
def _unique_org_slug(db: Session, name: str) -> str:
    ...
    candidate = base
    index = 2
    while db.query(Organisation).filter(Organisation.slug == candidate).first():
...
def _to_user_out(user: User) -> UserOut:
    ...
    return UserOut(
        ...
        organisation_licence_number=org.licence_number if org else None,
...
def register(payload: RegisterRequest, db: Session = Depends(get_db)):
    ...
    org = Organisation(
        ...
        slug=_unique_org_slug(db, payload.organisation_name),
    )
    db.add(org)
    db.flush()
...
def list_users(db: Session = Depends(get_db), admin: User = Depends(get_current_user)):
    ...
    return [_to_user_out(u) for u in query.all()]
